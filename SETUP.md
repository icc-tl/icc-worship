...
首頁每份歌單旁邊有「**樂手樂譜**」按鈕（灰色），**不需要登入**。

點進去會依照該週歌單的調性自動挑出對應樂譜，並可切換其他版本。
樂譜標籤用顏色區分：**藍**＝調性相符、**灰**＝樂譜沒標調性、**黃**＝不同調性
（黃色時上方會標示「此為 F 調，本週為 D 調」）。

右上角「**整份下載**」把當週所有歌的樂譜合成一份 PDF，可直接列印或存到平板 ——
也是主日早上網路出問題時的離線備援。
...
| 裝置 | 方式 | 說明 |
|---|---|---|
| 桌機 | PDF.js 繪製 | 有翻頁與縮放按鈕 |
| iPad / iPhone | 逐頁圖片 | 原生捲動與雙指縮放 |

iOS 走圖片是刻意的：內嵌 PDF 在 iOS 上**只顯示第一頁**，
而有 249 份樂譜是兩頁以上的。每一頁都預先轉成 JPEG 存在 R2，
所以 iPad 上捲得到所有頁面。原始 PDF 仍然保留，下載時拿到的是完整 PDF。
...
歌單的服事同工不是手key的，是讀「主日服事表」這份 Google Sheet：

```
https://docs.google.com/spreadsheets/d/10c_wxS-WybfjJ4QQigkMTihGlKl_1yxnDL0kk6GcFhc/
```

主領在編輯歌單選好日期，網站就去查那一天，把 **Worship Leader** 填進主領欄，
把 Vocal 1-3／Keyboard／Drum／Bass／Sound & Lighting 存成那份歌單的同工名單。

| 情況 | 行為 |
|---|---|
| 主領自己改過名字 | 自動同步**不會**蓋掉它；要換成表上的版本得按「從服事表同步」 |
| 那一天還沒排 | 顯示「服事表還沒排這一天」，其餘照舊手動輸入 |
| 表被改壞／關掉共用／連不上 | 安靜不顯示，不會跳錯誤，不影響做歌單 |

名單在**按儲存時就存進那份歌單**，之後不再依賴 Google —— 主日早上網路出問題也照常顯示，
也代表表格事後改了不會自動跟著變，要按「從服事表同步」。

### 需要留意的兩件事

**一、這份表不是我們的。** 擁有者是 `leongyc212@gmail.com`，目前的共用設定是
「知道連結的人可以檢視」。如果哪天改成不公開，這個功能就會安靜失效
（不會壞掉，只是不顯示）。欄位標題也不能亂改 —— 程式是靠
`Worship Leader`、`Vocal 1`、`Keyboard`、`Drum`、`Bass`、`Sound & Lighting`
這些標題文字去認欄位的，不是靠第幾欄。

**二、每年會開新分頁。** 程式會自己找：先試記住的分頁，再試程式裡寫死的，
最後掃過整份試算表所有分頁，挑出「標題有 Worship Leader 而且有那一年日期」的那一個。
所以 2027 年開新分頁時**不需要改程式**。真的找不到就退回手動輸入。
...
| 表上 | 網站上 |
|---|---|
| `Johnny / Zoe / Alex` | 主領：Johnny / Zoe / Alex |
| `Sean/Rudy(2)`、`Howard / Rudy (Bilingual)` | 括號註記去掉，只留人名 |
| `2/22/0206`（年份打錯） | 用該分頁最常見的年份補回 2026 |
| `Multitrack` | 原樣顯示 —— 那是伴奏帶，不是人 |
